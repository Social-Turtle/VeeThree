"""
PyTorch CNN Model with Rank-Based Encoding + Lowest-Passing Max Pooling
========================================================================
"""
...
class SequentialCNN(nn.Module):
    """
    CNN with rank-based encoding and lowest-passing max pooling.
    ...
    def __init__(self, encoding_type, n_pass, num_filters=NUM_FILTERS):
        ...
        # Custom pooling layer: selects winner based on lowest-passing signal strength
        ...
        # Input: 28x28
        # After conv1: 26x26, after custom pool: 13x13
    ...
    def forward(self, x):
        ...
        # x: (batch, 1, 28, 28) → (batch, 4/8, 26, 26)
        ...
        # Custom pooling: select winner based on lowest-passing signal strength
        x = self.custom_pool(encoded, raw_activations)  # → (batch, 4/8, 13, 13)
        
        # Conv2: Learned filters
        x = self.conv2(x)  # → (batch, 4/8, 11, 11)
        x = F.relu(x)
        x = F.max_pool2d(x, POOL_SIZE)  # → (batch, 4/8, 5, 5)
        
        # Flatten
        x = x.view(x.size(0), -1)  # → (batch, 100/200)
    ...
    def summary(self):
        """Print model summary."""
        print(f"\n{'='*60}")
        print(f"SequentialCNN Summary (with Lowest-Passing Pooling)")
        print(f"{'='*60}")
        ...
        print(f"Pool1: LowestPassingMaxPool (2x2) - winner = highest n_pass-th activation")
