"""Training script for the Edge-LUT hybrid MNIST classifier."""
...
# Import load_mnist first, before edge_lut_model modifies sys.path (edge_detection.py
# is imported from feature_engineering/ which is inserted at position 0 as a side-effect).
from main import load_mnist
from edge_lut_model import EdgeLUTModel, N_CLASSES, EDGE_STRIDE
...
def plot_confusion_matrix(model, images, labels, epoch):
    ...
    ax.set_ylabel('True')
    ax.set_title(f'Confusion Matrix — Edge-LUT Epoch {epoch}')
    ...
    plt.tight_layout()
    out_dir = os.path.join(_SCRIPT_DIR, 'visualizations', 'edge_lut')
...
def train(model, images, labels, epochs=5, print_every=500):
    """SGD training loop (mirrors lut_train.py, includes 'edge' seq2 key)."""
    n = len(images)
    global_step = 0
    ...
    for epoch in range(epochs):
        ...
        for i, idx in enumerate(indices):
            ...
            if global_step % print_every == 0:
                ...
                seq2_total   = (sum(seq2_local) + sum(seq2_global) + seq2_output + seq2_edge) / n_seen
                print(f"  epoch {epoch+1}/{epochs} step {i+1:>6}/{n} "
                      ...
                      f"total={total:.0f} "
                      f"| seq2s/img edge[{seq2_edge/n_seen:.0f}] "
                      f"local[{seq2_l_str}] "
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Train Edge-LUT hybrid MNIST classifier.")
    parser.add_argument("--stride", type=int, default=EDGE_STRIDE,
                        help="Edge patch stride (default: %(default)s).")
    args = parser.parse_args()

    print("=" * 50)
    print("Edge-LUT Hybrid Model - MNIST Training")
    print("=" * 50)
    print(f"Edge stride: {args.stride}")
    ...
    print("\nInitializing Edge-LUT model...")
    model = EdgeLUTModel(stride=args.stride)
